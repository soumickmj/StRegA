...
def smooth_tensor2D(tensor, kernel_size=8, sigma=3, channels=1):
...
def smooth_tensor3D(tensor, kernel_size=8, sigma=3, channels=1):
    ...
    # Create a x, y coordinate grid of shape (kernel_size, kernel_size, kernel_size, 3)
    cord = torch.arange(kernel_size)
    x_grid, y_grid, z_grid = torch.meshgrid(cord,cord,cord)
    ...
    gaussian_kernel = (1.0 / (2.0 * math.pi * variance)) * torch.exp(
        -torch.sum((xyz_grid - mean) ** 2.0, dim=-1) / (2.0 * variance)
    )
    # Make sure sum of values in gaussian kernel equals 1.
    gaussian_kernel = gaussian_kernel / torch.sum(gaussian_kernel)
    ...
    gaussian_filter = torch.nn.Conv3d(
        in_channels=channels,
        out_channels=channels,
        kernel_size=kernel_size,
        groups=channels,
...
def smooth_tensor(tensor, kernel_size=8, sigma=3, channels=1):
    if len(tensor.shape) == 5:
        return smooth_tensor3D(tensor, kernel_size, sigma, channels)
    else:
        return smooth_tensor2D(tensor, kernel_size, sigma, channels)
